...
你可以使用以下工具来收集信息：
1. `get_person_timeline`：获取某个人物的完整生平事件时间线。
...
请分析当前问题并提取实体，确定是人物关系、具体史实还是普通闲聊。
注意：如果用户提问中包含代词（如“他”、“他们”、“这”、“当时”等）或指代不清，必须结合历史对话将其替换为具体的历史人物、时间或地点，使重写后的 rewritten_question 能够作为一个完全独立的问题进行检索。
"""
...
请根据以下从多个信息搜集步骤和工具中检索出的历史事实数据，回答用户复杂的宏观历史问题。
